...
function twitter(hashtag, retweeted) {
    //Right now only serch is used api can alos be injected in constructor for wide usage
...
twitter.prototype.loadTweets = function (selector, retweeted) {
    ...
    $.ajax({
        ...
        var html = "";
        for (var i in resp) {
            html = html + "<div class='tweet'>" + error + "</div>";
        }
        $(selector).html(html);
    });
};
...
twitter.prototype.getQueryString = function () {
    //Can be extended to include other supported values using switch
    var queryString = "";
    if (this.hashTag) {
